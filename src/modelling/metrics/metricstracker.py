...
class MetricsTracker:
    ...
    def track_window(self, window_name: str):
        ...
        def decorator(func):
            ...
            def wrapper(*args, **kwargs):
                ...
                # Ensure window name is initialized in metrics storage
                if window_name not in self.metrics["energy"]:
                    self.metrics["energy"][window_name] = []
                ...
                try:
                    ...
                    if self.track_energy and window_name in self._active_windows:
                        ...
                        if self.verbose and self.inference:
                            print(f"\nInference:")
                            print(
                                f"  Inference Energy: {energy_measurement.total_energy:.2f}J"
                            )
                            print(f"  Inference Time: {energy_measurement.time:.2f}s")
    ...
    def _store_measurements(self, window_name: str, result: Dict, measurements: Dict):
        ...
        result = result or {}  # Ensure result is always a dict
        ...
        # Ensure energy tracking is set up for the window
        if window_name not in self.metrics["energy"]:
            self.metrics["energy"][window_name] = []
        ...
        if "memory" in measurements:
            for name, value in measurements["memory"].items():
                self.metrics["memory"].setdefault(name, []).append(value)
    ...
    def plot_metrics(self, save: bool = True):
        ...
        if self.metrics["train_loss"] and self.metrics["val_loss"]:
            num_plots += 1
        if self.track_energy and len(self.metrics["energy"].values()) > 0:
            num_plots += 1
        if self.track_memory and len(self.metrics["memory"].values()) > 0:
            num_plots += 1
        if len(self.metrics["energy"]["step"]) > 0:
            num_plots += 1

        # If no data to plot, return early
        if num_plots == 0:
        ...
        if self.metrics["train_loss"] and self.metrics["val_loss"]:
            axes[plot_idx].plot(
                epochs, self.metrics["train_loss"], label="Training Loss"
            )
            axes[plot_idx].plot(
                epochs, self.metrics["val_loss"], label="Validation Loss"
            )
        ...
        if self.track_energy and any(self.metrics["energy"].values()):
            plot_idx += 1
        ...
        if len(self.metrics["energy"]["step"]) > 0:  # Only plot steps if we have data
            plot_idx += 1
            axes[plot_idx].plot(
                step_energies,
                label="Energy (Step)",
        ...
        if self.track_memory and any(self.metrics["memory"].values()):
            plot_idx += 1
            print("Plotting memory metrics")
            print()
